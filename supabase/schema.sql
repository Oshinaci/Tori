...
-- Wallet Activity Policies
DROP POLICY IF EXISTS "Users can view their own wallet activity" ON public.wallet_activity;
CREATE POLICY "Users can view their own wallet activity" ON public.wallet_activity FOR SELECT USING (auth.uid() = user_id);

DROP POLICY IF EXISTS "Users can insert their own wallet activity" ON public.wallet_activity;
CREATE POLICY "Users can insert their own wallet activity" ON public.wallet_activity FOR INSERT WITH CHECK (auth.uid() = user_id);

DROP POLICY IF EXISTS "Users can update their own wallet activity" ON public.wallet_activity;
CREATE POLICY "Users can update their own wallet activity" ON public.wallet_activity FOR UPDATE USING (auth.uid() = user_id) WITH CHECK (auth.uid() = user_id);

DROP POLICY IF EXISTS "Users can delete their own wallet activity" ON public.wallet_activity;
CREATE POLICY "Users can delete their own wallet activity" ON public.wallet_activity FOR DELETE USING (auth.uid() = user_id);
